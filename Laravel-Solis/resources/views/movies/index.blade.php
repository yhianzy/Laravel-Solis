            ...
            <div class="row g-2 align-items-end">
                ...
                <div class="col-md-2">
                    ...
                    <select name="sort" class="form-select form-select-sm">
                        ...
                        <option value="rating_desc"{{ request('sort') == 'rating_desc'         ? 'selected' : '' }}>Rating (High)</option>
                        <option value="rating_asc" {{ request('sort') == 'rating_asc'          ? 'selected' : '' }}>Rating (Low)</option>
                    </select>
                </div>
                <div class="col-md-1 d-flex gap-1">
                    <button type="submit" class="btn btn-primary btn-sm w-100"><i class="bi bi-funnel"></i></button>
                    <a href="{{ route('movies.index') }}" class="btn btn-outline-secondary btn-sm w-100"><i class="bi bi-x"></i></a>
                </div>
            </div>
            <div class="mt-2">
                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="checkbox" name="favorites" id="favOnly" value="1" {{ request('favorites') ? 'checked' : '' }} onchange="this.form.submit()">
                    <label class="form-check-label" for="favOnly" style="font-size:0.82rem"><i class="bi bi-star-fill text-warning me-1"></i>Favorites only</label>
                </div>
            </div>
        </form>
    </div>
            ...
            <table class="table table-hover mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            @if($movie->poster)
                                <img src="{{ str_starts_with($movie->poster,'http') ? $movie->poster : Storage::url($movie->poster) }}"
                                    class="rounded" width="40" height="55" style="object-fit:cover">
                            @else
                                <div class="rounded d-flex align-items-center justify-content-center" style="width:40px;height:55px;background:linear-gradient(135deg,#6366f1,#06b6d4)">
                                    <i class="bi bi-film text-white" style="font-size:1rem"></i>
                                </div>
                            @endif
                        </td>
                        <td>
                            <div class="fw-semibold" style="cursor:pointer" onclick="showDetail({{ $movie->id }})">
                                {{ $movie->title }}
                                @if($movie->is_favorite)<i class="bi bi-star-fill text-warning ms-1" style="font-size:0.75rem"></i>@endif
                            </div>
                            @if($movie->duration)<div class="text-muted" style="font-size:0.75rem">{{ $movie->duration }} min</div>@endif
                        </td>
                        ...
                        <td class="text-muted" style="font-size:0.82rem">{{ $movie->director ?? '—' }}</td>
...
<div id="cardView" style="display:none">
    @forelse($movies as $movie)
    <div class="col" style="display:inline-block;width:200px;vertical-align:top;margin:0 10px 20px 0">
        <div class="movie-card" onclick="showDetail({{ $movie->id }})">
            @if($movie->poster)
                <img src="{{ str_starts_with($movie->poster,'http') ? $movie->poster : Storage::url($movie->poster) }}"
                    style="width:100%;height:200px;object-fit:cover">
            @else
                <div class="poster"><i class="bi bi-film"></i></div>
            @endif
            <div class="card-body">
                <div class="movie-title">{{ $movie->title }}</div>
                <div class="d-flex justify-content-between align-items-center mt-1">
                    <span class="badge-genre" style="font-size:0.68rem">{{ $movie->genre }}</span>
                    <span class="text-muted" style="font-size:0.75rem">{{ $movie->year }}</span>
                </div>
                <div class="d-flex justify-content-between align-items-center mt-2">
                    @php $sc = ['Watched'=>'badge-watched','Unwatched'=>'badge-unwatched','Watchlist'=>'badge-watchlist']; @endphp
                    <span class="{{ $sc[$movie->status] ?? 'badge-unwatched' }}" style="font-size:0.68rem">{{ $movie->status }}</span>
                    @if($movie->rating)<span class="badge-rating" style="font-size:0.68rem"><i class="bi bi-star-fill"></i> {{ $movie->rating }}</span>@endif
                </div>
                <div class="d-flex justify-content-between align-items-center mt-2" onclick="event.stopPropagation()">
                    <form method="POST" action="{{ route('movies.favorite', $movie) }}">
                        @csrf
                        <button class="fav-btn {{ $movie->is_favorite ? 'active' : '' }}">
                            <i class="bi bi-star{{ $movie->is_favorite ? '-fill' : '' }}"></i>
                        </button>
                    </form>
                    <div class="d-flex gap-1">
                        <a href="{{ route('movies.edit', $movie) }}" class="btn btn-sm btn-outline-primary py-0 px-1"><i class="bi bi-pencil" style="font-size:0.75rem"></i></a>
                        <form method="POST" action="{{ route('movies.destroy', $movie) }}" onsubmit="return confirm('Delete?')">
                            @csrf @method('DELETE')
                            <button class="btn btn-sm btn-outline-danger py-0 px-1"><i class="bi bi-trash" style="font-size:0.75rem"></i></button>
                        </form>
                    </div>
                </div>
            </div>
                ...
                <div class="modal-body p-4">
                    <div class="row g-3">
                        <div class="col-md-8">
                            ...
                            <input type="text" name="title" class="form-control @error('title') is-invalid @enderror" value="{{ old('title') }}" placeholder="e.g. The Dark Knight" required>
                        ...
                        <div class="col-md-8">
                            <label class="form-label">Cast (main actors)</label>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="poster_url" class="form-control" value="{{ old('poster_url') }}" placeholder="https://...">
                        </div>
                        <div class="col-12">
...
function showDetail(id) {
    ...
    if (!el) return;
    const d = el.dataset;
    const statusClass = {Watched:'badge-watched',Unwatched:'badge-unwatched',Watchlist:'badge-watchlist'};
    const poster = d.poster
        ? `<img src="${d.poster}" style="width:100%;height:280px;object-fit:cover">`
        : `<div style="width:100%;height:280px;background:linear-gradient(135deg,#6366f1,#06b6d4);display:flex;align-items:center;justify-content:center"><i class="bi bi-film text-white" style="font-size:4rem;opacity:0.4"></i></div>`;
        ...
        <div class="row g-0">
            ...
            <div class="col-md-8 p-4">
                <div class="d-flex gap-2 flex-wrap mb-3">
                    ...
                    <span class="${statusClass[d.status]||'badge-unwatched'}">${d.status}</span>
                    ${d.favorite==='1' ? '<span style="color:#f59e0b"><i class="bi bi-star-fill"></i> Favorite</span>' : ''}
                </div>
                <div class="row g-2 mb-3">
                    <div class="col-6"><div style="font-size:0.7rem;font-weight:600;color:#94a3b8;text-transform:uppercase">Year</div><div style="font-weight:600">${d.year}</div></div>
                    <div class="col-6"><div style="font-size:0.7rem;font-weight:600;color:#94a3b8;text-transform:uppercase">Rating</div><div style="font-weight:600">${d.rating !== '—' ? '⭐ '+d.rating+'/10' : '—'}</div></div>
                    <div class="col-6"><div style="font-size:0.7rem;font-weight:600;color:#94a3b8;text-transform:uppercase">Duration</div><div style="font-weight:600">${d.duration}</div></div>
                    <div class="col-6"><div style="font-size:0.7rem;font-weight:600;color:#94a3b8;text-transform:uppercase">Language</div><div style="font-weight:600">${d.language}</div></div>
                    <div class="col-12"><div style="font-size:0.7rem;font-weight:600;color:#94a3b8;text-transform:uppercase">Director</div><div style="font-weight:600">${d.director}</div></div>
                    <div class="col-12"><div style="font-size:0.7rem;font-weight:600;color:#94a3b8;text-transform:uppercase">Cast</div><div style="font-weight:600;font-size:0.875rem">${d.cast}</div></div>
                </div>
